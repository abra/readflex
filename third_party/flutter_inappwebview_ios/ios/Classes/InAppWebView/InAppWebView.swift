...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate,
...
#if DEBUG
        NSLog(
            "[reader-context-menu-native] buildMenu url=%@ disableContextMenu=%@",
            url?.absoluteString ?? "nil",
            String(describing: settings?.disableContextMenu)
        )
#endif

        if #available(iOS 16.0, *) {
            var shouldHideDefaultSystemItems = settings?.disableContextMenu == true
            if let menu = contextMenu {
                ...
                if let contextMenuSettingsMap = menu["settings"] as? [String: Any?] {
                    ...
                    if contextMenuSettings.hideDefaultSystemContextMenuItems {
                        shouldHideDefaultSystemItems = true
                    }
                }
            }
...
#if DEBUG
        NSLog(
            "[reader-context-menu-native] willPresentEditMenu url=%@ disableContextMenu=%@ interactions=%@",
            url?.absoluteString ?? "nil",
            String(describing: settings?.disableContextMenu),
            readflexInteractionSummary()
        )
#endif
        if settings?.disableContextMenu == true {
            dismissSystemEditMenu()
            return
        }
        onCreateContextMenu()
    }
    ...
    @available(iOS 16.0, *)
    private func dismissSystemEditMenu() {
        let editMenuInteractions = readflexEditMenuInteractions()
#if DEBUG
        NSLog(
            "[reader-context-menu-native] dismissSystemEditMenu count=%ld interactions=%@",
            editMenuInteractions.count,
            readflexInteractionSummary()
        )
#endif
        let dismiss: () -> Void = { [weak self] in
            guard let self = self else { return }
            self.readflexEditMenuInteractions()
    ...
    @available(iOS 16.0, *)
    private func readflexEditMenuInteractions() -> [UIEditMenuInteraction] {
        var result: [UIEditMenuInteraction] = []
        var pendingViews: [UIView] = [self]
        while let view = pendingViews.popLast() {
            result.append(contentsOf: view.interactions.compactMap {
                $0 as? UIEditMenuInteraction
            })
    ...
    @available(iOS 16.0, *)
    private func readflexInteractionSummary() -> String {
        var entries: [String] = []
        var pendingViews: [UIView] = [self]
        while let view = pendingViews.popLast() {
            let owner = String(describing: type(of: view))
            for interaction in view.interactions {
                entries.append("\(owner):\(String(describing: type(of: interaction)))")
            }
            pendingViews.append(contentsOf: view.subviews)
        }
